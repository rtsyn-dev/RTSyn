...
use crate::protocol::{PluginSummary, RuntimeSettingsOptions};
...
fn handle_client(stream: UnixStream, state: &mut DaemonState) -> Result<(), String> {
    ...
    let response = match request {
        DaemonRequest::PluginList => plugin_list(&state.catalog),
        ...
        DaemonRequest::PluginRebuild { name } => plugin_rebuild(&mut state.catalog, name),
