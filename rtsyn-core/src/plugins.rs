...
pub trait PluginMetadataSource {
    fn query_plugin_metadata(
        ...
        library_path: &str,
        timeout: Duration,
    ) -> Option<(Vec<String>, Vec<String>, Vec<(String, f64)>, Option<DisplaySchema>, Option<UISchema>)>;
...
impl PluginManager {
    ...
    pub fn install_plugin_from_folder(
        ...
        let manifest: PluginManifest = toml::from_str(&data)
            .map_err(|err| format!("Invalid plugin.toml: {err}"))?;
        if manifest.kind == "comedi_daq" && !cfg!(feature = "comedi") {
    ...
    pub fn remove_plugins_by_kind_from_workspace(
        ...
        self.available_plugin_ids.extend(removed_ids.iter().copied());
        removed_ids
    }
}
...
impl PluginCatalog {
    ...
    pub fn install_plugin_from_folder<P: AsRef<Path>>(
        ...
        let (metadata_inputs, metadata_outputs, metadata_variables, mut display_schema, ui_schema) =
            if let Some(ref lib_path) = library_path {
                let lib_path_str = lib_path.to_string_lossy();
                match metadata.query_plugin_metadata(lib_path_str.as_ref(), Duration::from_secs(2)) {
                    Some((inputs, outputs, vars, schema, ui_sch)) => (inputs, outputs, vars, schema, ui_sch),
                    None => (vec![], vec![], vec![], None, None),
                }
            } else {
                (vec![], vec![], vec![], None, None)
    ...
    pub fn uninstall_plugin_by_kind(&mut self, kind_or_name: &str) -> Result<InstalledPlugin, String> {
        let index = self
            .manager
            .installed_plugins
            .iter()
            .position(|p| p.manifest.kind == kind_or_name || p.manifest.name.eq_ignore_ascii_case(kind_or_name))
    ...
    pub fn remove_plugins_by_kind_from_workspace(
        ...
        self.manager.available_plugin_ids.extend(removed_ids.iter().copied());
        removed_ids
    }
    ...
    pub fn add_installed_plugin_to_workspace(
        ...
        let installed = self
            ...
            .installed_plugins
            .iter()
            .find(|p| p.manifest.kind == kind_or_name || p.manifest.name.eq_ignore_ascii_case(kind_or_name))
            .cloned()
        ...
        let loads_started = metadata
            .query_plugin_behavior(
                ...
                installed.library_path.as_ref().map(|p| p.to_string_lossy()).as_deref(),
                Duration::from_secs(2),
            )
            .map(|b| b.loads_started)
            .unwrap_or(false);
